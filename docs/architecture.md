...
│  handlers/impact.py  → Impact metrics                         │
│  handlers/roi.py     → Return on investment                   │
│  handlers/health.py  → Health check                           │
│                                                               │
│  services/otlp_parser.py    → OTEL payload parsing            │
│  services/git_metrics.py    → Git metrics                     │
...
│  ImpactTab        → Lead time, AI share, correlations         │
│  RoiTab           → Investment, cost/PR, adoption             │
...
│                           → services/git_metrics.py
│                           → functions/claude_code/normalize.py
├── handlers/roi.py         → services/mysql_db.py
│                           → services/otlp_parser.py
│                           → functions/claude_code/normalize.py
└── services/
